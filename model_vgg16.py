...
class vgg16:
    def __init__(self, imgs, _dropout = 1.0, weights=None, sess=None):
        self.imgs = imgs
        self.convlayers()
        self.fc_layers(_dropout)
        self.probs = self.fc1#tf.nn.softmax(self.fc3l)  #计算softmax层输出
    ...
    def fc_layers(self,_dropout):
        ...
        with tf.name_scope('fc1') as scope:
            ...
            self.fc1 = tf.nn.dropout(self.fc1, keep_prob=_dropout)
        ...
        with tf.name_scope('fc2') as scope:
            ...
            self.fc2 = tf.nn.dropout(self.fc2, keep_prob=_dropout)

            self.parameters += [fc2w, fc2b]

        # fc3
        with tf.name_scope('fc3') as scope:
            fc3w = tf.Variable(tf.truncated_normal([fc_size, nclasses],
                                                         dtype=tf.float32,
                                                         stddev=1e-2), name='weights')
            fc3b = tf.Variable(tf.constant(1.0, shape=[nclasses], dtype=tf.float32),
                                 trainable=True, name='biases')
            self.fc3l = tf.nn.bias_add(tf.matmul(self.fc2, fc3w), fc3b)
            #self.fc3l = tf.nn.dropout(self.fc3l, keep_prob=_dropout)
...
'''if __name__ == '__main__':
    ...
    img1 = imread('img1.jpg', mode='L')    #载入需要判别的图片
    ...
    prob = sess.run(vgg.probs, feed_dict={imgs: [img1, img2]})  
