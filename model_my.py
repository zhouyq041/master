...
# 生成V3网络的卷积部分
def inception_v3_base(inputs, inputs2 = None,scope=None):
  '''
  Args:
  inputs：输入的tensor
  scope：包含了函数默认参数的环境
...
----------------------------------------------------------------------    
      ...
      print 'net after Inception 3 end:\n',net,'\n'
  ...
  with tf.variable_scope(scope, 'InceptionV3', [inputs, num_classes], # 定义参数默认值
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout], # 定义标志默认值
      ...
      '''with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
        ...
        with tf.variable_scope('AuxLogits'):
          ...
          aux_logits = slim.conv2d(
              aux_logits, 768, [aux_logits.get_shape()[1], aux_logits.get_shape()[1]],#modified by zhou
      ...
      with tf.variable_scope('Logits'):
        ...
        print 'net after last pool:\n',net,'\n'
        if inputs2 != None:
          inputs2 = tf.reshape(inputs2,shape=[-1,1,3,19])
          inputs2 = slim.conv2d(inputs2, 512, [1, 3],padding = 'VALID', scope='inputs2')
          net = tf.concat([net,inputs2],3)
          
        if inputs3 == None:
          pass#net = tf.concat([net,inputs2],3)
          #net = tf.concat([net,inputs3],3)
        else:
          net = slim.conv2d(net, 512, [1, 1],padding = 'VALID', scope='inputs_net')
          #inputs3 = slim.conv2d(inputs3, 512, [1, 1],padding = 'VALID', scope='inputs_3')
          net = tf.concat([net,inputs2,inputs3],3)
        print 'net after add all inputs:\n',net,'\n'
        net = slim.dropout(net, keep_prob=dropout_keep_prob, scope='Dropout_1b')
        end_points['PreLogits'] = net

        # 激活函数和规范化函数设为空
        logits = slim.conv2d(net, num_classes, [1, 1], activation_fn=tf.nn.relu, #modified by zhou
                             normalizer_fn=slim.batch_norm, scope='Conv2d_1c_1x1') # 1x1x1000
