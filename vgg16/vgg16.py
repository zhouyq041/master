...
if __name__ == '__main__':
    ...
    img1 = imread('img1.jpg')    #载入需要判别的图片
    ...
    the_images = []
    test_cartoon_dir = 'test_cartoon/11'
    for files in os.listdir(test_cartoon_dir):
        the_images.append(test_cartoon_dir+'/'+files)
    ...
    for the_image in the_images:
        ...
        the_image = imread(the_image)
        the_image = imresize(the_image, (224, 224))
        prob = sess.run(vgg.probs, feed_dict={imgs: [the_image]})  
        
            #print the_image
        for pro in prob:
            preds = (np.argsort(-pro))[0:2]  #取出top5的索引
            if(pro[preds[0]] > 0.7):
                good_count += 1
